/* * To change this license header, choose License Headers in Project Properties. * To change this template file, choose Tools | Templates * and open the template in the editor. */package online.medicine.donation.system;import com.mysql.jdbc.Connection;import com.mysql.jdbc.PreparedStatement;import com.toedter.calendar.JDateChooser;import java.awt.Color;import java.awt.Container;import java.awt.Cursor;import java.awt.Font;import java.awt.event.ActionEvent;import java.awt.event.ActionListener;import java.awt.event.FocusEvent;import java.awt.event.FocusListener;import java.sql.DriverManager;import java.sql.ResultSet;import java.sql.SQLException;import java.text.DateFormat;import java.text.SimpleDateFormat;import java.time.LocalDate;import java.time.LocalTime;import java.time.format.DateTimeFormatter;import java.util.Date;import javax.swing.JButton;import javax.swing.JComboBox;import javax.swing.JFrame;import javax.swing.JLabel;import javax.swing.JOptionPane;import javax.swing.JTextField;

/**
 * CollectMedicine class - Allows executives to collect donated medicines
 * Handles collection details, date, time, and executive information
 * @author Hp
 */
public class CollectMedicine extends JFrame {
    ...
    private JLabel namel, enamel, eml, n, genderl, agel, contactl, locationl;
    private JLabel datel, mnfl, expl, typel, numl, med, time;
    
    // UI Components - Input Fields
    private JTextField nametf, emailtf, contacttf, locationtf, gendertf, agetf, idtf;
    private JTextField enamef, etf, nf, mnfltf, expltf, typetf, numtf, timetf;
    private JComboBox nameltf;
    private JDateChooser datetf;
    
    // UI Components - Buttons
    private JButton homepage, user, logout, search, assign, viwngo, viewuser;
    private JButton viewexecutive, collect, viewmedicine, donate, buymed;
    ...
    /**
     * Create main content area with collection form
     */
    private void createMainContentArea(Font font1, Cursor cursor) {
        ...
        title2 = new JLabel("Please enter medicine name and click search button. Then enter collect date and collect time.");
        title2.setFont(font4);
        title2.setBounds(420, 45, 1900, 27);
        label1.add(title2);

        // Create medicine selection section
        createMedicineSelectionSection(font4, cursor, label1);

        // Create executive details section
        createExecutiveDetailsSection(font3, font4, label1);

        // Create collection details section
        createCollectionDetailsSection(font4, label1);
     ...
     * Create medicine selection section
     */
    private void createMedicineSelectionSection(Font font4, Cursor cursor, JLabel label1) {
        ...
        search.setFont(new Font("Arial", Font.BOLD, 24));
     ...
     * Create collection details section
     */
    private void createCollectionDetailsSection(Font font4, JLabel label1) {
        ...
        med.setFont(new Font("Arial", Font.BOLD, 26));
    ...
    /**
     * Add focus listener for time field placeholder
     */
    ...
    private class Handler implements ActionListener {
        ...
        /**
         * Handle search for medicine and executive details
         */
        private void handleSearch(String name) {
            if (name.isEmpty() || enamef.getText().isEmpty() || etf.getText().isEmpty()) {
                ...
                searchMedicineDetails(name);
                searchExecutiveDetails();
            }
        }

        /**
         * Search medicine details from donate medicine table
         */
        private void searchMedicineDetails(String name) {
            int a = 0;
            try {
        ...
        /**
         * Search executive details from executive table
         */
        private void searchExecutiveDetails() {
            try {
        ...
        /**
         * Handle medicine collection
         */
        private void handleCollectMedicine(String name) {
            if (datetf.getDate() == null || timetf.getText().isEmpty() || agetf.getText().isEmpty()
                    || gendertf.getText().isEmpty() || contacttf.getText().isEmpty()
                    || locationtf.getText().isEmpty() || name.isEmpty() || enamef.getText().isEmpty()
                    || etf.getText().isEmpty() || mnfltf.getText().isEmpty() || expltf.getText().isEmpty()
                    || numtf.getText().isEmpty() || typetf.getText().isEmpty()) {
                JOptionPane.showMessageDialog(null, "Fill up all information", "Warning", JOptionPane.ERROR_MESSAGE);
            } else {
                validateAndInsertCollection(name);
            }
        }

        /**
         * Validate collection date and insert record
         */
        private void validateAndInsertCollection(String name) {
            if (datetf.getDate() != null && !timetf.getText().isEmpty()) {
                ...
                try {
                    ...
                    if (date2.before(date1)) {
                        JOptionPane.showMessageDialog(null, "Please enter collect date less than or equal to current date");
                    } else {
                        insertCollectionRecord(name, dates, times);
                    }
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(null, "Collect medicine unsuccessfully: " + ex);
                }
            }
        }

        /**
         * Insert collection record into database
         */
        private void insertCollectionRecord(String name, String dates, String times) {
    ...
    /**
     * Load medicine names from donate medicine table into combo box
     */
